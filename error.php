...
<div class="content">
        ...
        <div id="social">
        	<div id="twitter">
            	<iframe allowtransparency="true" frameborder="0" scrolling="no" src="http://platform.twitter.com/widgets/tweet_button.html?url=<?=rawurlencode($properties[$_GET['pageid']]['url'])?>&text=<?=rawurlencode($tweetData)?>&count=none" style="width:65px; height:22px;"></iframe>
            	<!--<a href="http://twitter.com/share" class="twitter-share-button" data-text="<?=$tweetData?>" data-url="<?=$properties[$_GET['pageid']]['url']?>" data-count="none">Tweet</a><script type="text/javascript" src="http://platform.twitter.com/widgets.js"></script>-->
            </div>
            <div id="fb_like">
            	<fb:like href="http://www.facebook.com/stationcasinos?sk=app_210231899020925" send="false" layout="button_count" width="90" show_faces="false" font="arial"></fb:like>
            </div>
            <div id="fb_share">
    ...
    <?php   
		...
		switch($errorcode) {  
			...
			case "4" : $strError = "Invalid information was provided. Please check your email and all other fields and try again. (".$errorcontrol.")"; break;  
        ...
        echo '<p style="color:red;">Error: '.$strError . ' ('.$errorcode.')<p>'; 
	?>
    </div>
	...
	<div class="content" style="padding-bottom:20px;border-bottom:1px solid black;">
    	<form id="subscribeForm" action="http://cl.exct.net/subscribe.aspx?lid=120158"  name="subscribeForm" method="post" onsubmit="return checkForm();">
        	<input type="hidden" name="thx" value="http://www.digitalroyaltyapps.com/clients/stations/35th/thankyou.php?pageid=<?=$_GET['pageid']?>" />  
            <input type="hidden" name="err" value="http://www.digitalroyaltyapps.com/clients/stations/35th/error.php?pageid=<?=$_GET['pageid']?>" />  
            ...
            <input type="hidden" name="Hidden Field" id="hf" value="<?=$properties[$_GET['pageid']]['name']?>" />
            <br class="clear" />
